...
export function App() {
  ...
  // Load state from local storage or fall back to default database records
  const [feedItems, setFeedItems] = useState(() => {
    const saved = localStorage.getItem('wots_hot_flames');
    if (saved) {
      try {
  ...
  // Sync to local storage whenever flame state updates
  useEffect(() => {
    localStorage.setItem('wots_hot_flames', JSON.stringify(feedItems));
  }, [feedItems]);

  // Handle the action of firing up / flaming an article up
